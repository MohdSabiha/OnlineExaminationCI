...
class test_model extends CI_Model {
    ...
    {
        $this->datatables->select('a.id_test, a.token, a.nama_test, b.nama_matkul, a.jumlah_soal, CONCAT(a.tgl_mulai, " <br/> (", a.waktu, " Minute)") as waktu, a.jenis');
        $this->datatables->from('m_test a');
    ...
    public function getListtest($id, $kelas)
    {
        $this->datatables->select("a.id_test, e.nama_dosen, d.nama_kelas, a.nama_test, b.nama_matkul, a.jumlah_soal, CONCAT(a.tgl_mulai, ' <br/> (', a.waktu, ' Minute)') as waktu,  (SELECT COUNT(id) FROM h_test h WHERE h.mahasiswa_id = {$id} AND h.test_id = a.id_test) AS ada");
        $this->datatables->from('m_test a');
        $this->datatables->join('matkul b', 'a.matkul_id = b.id_matkul');
    ...
    public function getSoal($id)
    {
        ...
        $order = $test->jenis==="Random" ? 'rand()' : 'id_soal';

        $this->db->select('id_soal, soal, file, tipe_file, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, jawaban');
        $this->db->from('tb_soal');
        $this->db->where('dosen_id', $test->dosen_id);
        $this->db->where('matkul_id', $test->matkul_id);
        $this->db->order_by($order);
        $this->db->limit($test->jumlah_soal);
    ...
    {
        $this->datatables->select('b.id_test, b.nama_test, b.jumlah_soal, CONCAT(b.waktu, " Minute") as waktu, b.tgl_mulai');
        $this->datatables->select('c.nama_matkul, d.nama_dosen');
        $this->datatables->from('h_test a');
        $this->datatables->join('m_test b', 'a.test_id = b.id_test');
        $this->datatables->join('matkul c', 'b.matkul_id = c.id_matkul');
        $this->datatables->join('dosen d', 'b.dosen_id = d.id_dosen');
        $this->datatables->group_by('b.id_test');
        if($nip !== null){
    ...
    public function HsltestById($id, $dt=false)
    {
        ...
        $this->$db->join('kelas b', 'a.kelas_id=b.id_kelas');
        $this->$db->join('jurusan c', 'b.jurusan_id=c.id_jurusan');
        $this->$db->join('h_test d', 'a.id_mahasiswa=d.mahasiswa_id');
        $this->$db->where(['d.test_id' => $id]);
        return $this->$db->$get();
    }
